            ...
            <div class="row product_tbl block animatable bounceInLeft">
               ...
               <table>
                  ...
                  @foreach($cart_details as $cart_detail)
                   <?php 
                    $total = $total + ($cart_detail->price*$cart_detail->qty - (($cart_detail->discount/100)*$cart_detail->price*$cart_detail->qty));
                    $discount_price = $discount_price + ($cart_detail->discount/100*$cart_detail->price)*$cart_detail->qty;
...
<!--                             <img src="img/1.png">-->
                     ...
                     @if(isset($cart_detail->discount) && !empty($cart_detail->discount) && $cart_detail->discount != 0.00)
                         <span>{{ number_format(($cart_detail->discount/100)*$cart_detail->price*$cart_detail->qty, 2) }} </span>                         
                         @endif  
                     </td>
                     <td class="color_p">
...
<!--                        <p> <input type="number" name="quantity" min="1" max="5" placeholder="1"></p>-->
                     ...
                     <td class="totl_p"><i class="fa fa-inr" aria-hidden="true"></i> 
                         {{ number_format($cart_detail->price*$cart_detail->qty - (($cart_detail->discount/100)*$cart_detail->price*$cart_detail->qty),2) }}  
<!--                         <a href="#" class="remov">X</a>-->
                         <a href="javascript::void(0);" class="remov remove" id="remove_id" data-id = "{{ $cart_detail->id }}">X</a>
                     </td>
                  </tr>
                  @endforeach
               </table>
            ...
            <div class="row totl_dlt ">
               ...
               <div class="col-md-6">
                  <table class="block animatable moveUp">
                   ...
                   @if(Auth::check())
                        <a href="{{ URL::to('checkout') }}" class="btn_continue1 btn_continue3 block animatable bounceInLeft">
                     ...
                     <br /><br /><br />
                     <a href="{{ URL::to('guest_checkout') }}" class="btn_continue1 btn_continue3 block animatable bounceInLeft">Continue as a guest</a>
                     @endif

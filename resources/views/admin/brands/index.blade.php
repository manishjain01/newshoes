...
<section class="content-header">
  ...
  {!!  Html::decode(Html::link(route('admin.brands.create'),"<i class='fa  fa-plus'></i>",['class'=>'btn btn-primary','data-toggle'=>'tooltip','title'=>trans('Add')])) !!}				  
...
<!--                  <th  width="15%" align="center">{{trans('admin.PROFILE_IMAGE')}}</th>-->
			  <th width="5%">S.No</th>
              <th width="15%">@sortablelink('brand_name', 'Brand')</th>
           ...
           <tr>
              ...
              <td>{{ ucfirst($brand->brand_name)}}</td>
               <td>{{ date_val($brand->created_at,DATE_FORMATE ) }}</td>
              <td>
                  @if($brand->status == 1)
                    Active
                  @else
              ...
              <td>
				  {!!  Html::decode(Html::link(route('admin.brands.edit', $brand->id),"<i class='fa  fa-edit'></i>",['class'=>'btn btn-primary','data-toggle'=>'tooltip','title'=>trans('admin.EDIT')])) !!}				  
           ...
           {!! $brands->appends(Input::all('brands'))->render() !!}
        </div><!-- /.box-body -->
      </div><!-- /.box -->
    </div><!-- /.col -->
  </div><!-- /.row -->
